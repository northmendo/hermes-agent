...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        sys.exit(1)
    zip_url = (
        f"https://github.com/northmendo/hermes-agent/archive/refs/heads/{branch}.zip"
    )
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # venv created inside a previous source checkout).  If we blindly
    # trust the .git directory we end up pulling source commits the
    ...
    # Build git command once — reused for the update itself.
    git_cmd = ["git"]
    if sys.platform == "win32":
        git_cmd = ["git", "-c", "windows.appendAtomically=false"]
